...
问题1的解决思路：让页B（执行视图）上**不是jmp，而是INT3（0xCC）**。执行到INT3会触发#BP异常，我们拦截#BP，在VMEXIT中把Guest RIP改成跳板地址（由Hypervisor接管跳转控制，避免在影子页中直接暴露跳转指令/目标地址），然后立刻切回页A。这样页B暴露的时间窗口被压缩到"执行一条单字节INT3的瞬间。
